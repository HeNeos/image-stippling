...
pub fn generate_stippling(
    ...
    if bright_pixels.is_empty() {
        while points.len() < num_points {
            let x: f32 = rng.gen_range(0, width) as f32;
            let y: f32 = rng.gen_range(0, height) as f32;
            points.push((x, y));
        }
    } else {
        while points.len() < num_points {
            let idx: usize = rng.gen_range(0, bright_pixels.len());
    ...
    for _ in 0..iterations.min(200) {
        ...
        for (i, cell) in cells.iter().enumerate() {
            if i >= points.len() || cell.points().is_empty() {
                if i < points.len() {
                    new_points.push(points[i]);
                    new_average_weights[i] = 0.0;
                }
                continue;
            }
            ...
            if sum_weight > 0.0 {
                ...
                new_average_weights[i] = avg;
                if avg > new_max_weight {
                    new_max_weight = avg;
                }
            } else {
                new_points.push(points[i]);
